...
# ============================================================
# 3. Per-boat normalization (divide by 30)
# ============================================================
def normalize_boat(data):
    """Divide Tp/Ts by 30. States (u,v,r,du,dv,dr) NOT normalized."""
    out = {}
    # states: keep original values (not normalized)
    for c in ['u', 'v', 'r', 'du', 'dv', 'dr']:
        out[c] = data[c]
    # controls: divide by 30
    out['Tp'] = data['Tp'] / CTRL_SCALE
    out['Ts'] = data['Ts'] / CTRL_SCALE
    out['x'] = data['x']
    out['y'] = data['y']
    out['psi'] = data['psi']
    out['N'] = data['N']
...
# ============================================================
def predict_physical(params, u, v, r, Tp, Ts):
    """Forward-pass: divide Tp/Ts by 30, apply model. States NOT normalized."""
    # Divide controls by 30
    Tp_n = Tp / CTRL_SCALE
    Ts_n = Ts / CTRL_SCALE
...
def simulate_with_params(params, raw, dt, N):
    ...
    for k in range(1, N):
        ...
        du, dv, dr = predict_physical(params, uc, vc, rc, Tp, Ts)
...
def run_cv_fold(train_idx, test_idx, raw, norm, X_list, y_list, n_list):
    ...
    for k in train_idx:
        d  = raw[k]
        Ns = d['N']
        xs, ys, psis, _, _, _ = simulate_with_params(theta_fold, d, DT, Ns)
...
def main():
    ...
    print('=' * 110)
    print(f'  {"训练船":<14}{"测试船":<8}{"x MSE":>28}{"y MSE":>28}{"psi MSE":>28}')
    print('-' * 110)
    ...
    for result in cv_results:
        ...
        x_str   = f'{result["test_mse"]["x"]:.3f} ({self_x_vals[0]:.3f},{self_x_vals[1]:.3f})'
        y_str   = f'{result["test_mse"]["y"]:.3f} ({self_y_vals[0]:.3f},{self_y_vals[1]:.3f})'
        psi_str = f'{result["test_mse"]["psi"]:.3f} ({self_psi_vals[0]:.3f},{self_psi_vals[1]:.3f})'
        
        print(f'  {train_str:<14}{test_str:<8}{x_str:>28}{y_str:>28}{psi_str:>28}')
    ...
    # ----- 6. plot using fold 2 (USV1+USV3 -> USV2) as reference -----
    ref_result = cv_results[1]  # Fold 2: USV1+USV3 -> USV2
    theta = ref_result['theta']
    
    # Build self_sim for plotting
    self_sim = {}
    for k in [0, 2]:  # USV1, USV3
        d  = raw[k]
        Ns = d['N']
        xsk, ysk, _, usk, vsk, rsk = simulate_with_params(theta, d, DT, Ns)
    ...
    cv_table[BOAT_NAMES[1]] = {
        'role': 'test',
        'mse':  regression_mse(theta, X_list[1], y_list[1], n_list[1]),
    }
...
def make_all_figures(raw, norm, theta, X_list, y_list, n_list,
    ...
    for i in range(N_BOATS):
        d = raw[i]
        Ns = d['N']
        x_sim, y_sim, _, _, _, _ = simulate_with_params(theta, d, dt, Ns)
    ...
    for i in range(N_BOATS):
        d = raw[i]
        Ns = d['N']
        x_sim, y_sim, psi_sim, u_sim, v_sim, r_sim = simulate_with_params(theta, d, dt, Ns)
        t = np.arange(Ns) * dt
    ...
    fB.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, 1.01),
